...
function ScrollHint({ targetRef }: { targetRef: React.RefObject<HTMLDivElement | null> }) {
...
function spearman(xs: number[], ys: number[]) {
  const rx = rank(xs);
  const ry = rank(ys);
  return pearson(rx, ry); // p ~ t-розподіл (добра апроксимація при n>=10)
}
...
function colorForR(r: number | null) {
  ...
  const t = (r + 1) / 2; // 0..1
  const c1 = { r: 37, g: 99, b: 235 };   // #2563eb
